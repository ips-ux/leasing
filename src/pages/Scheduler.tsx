...
export const Scheduler = () => {
  ...
  return (
    <div className="space-y-6 h-[calc(100vh-2rem)] flex flex-col">
      {/* Header — top row: title left, controls right */}
      <div className="flex justify-between items-start gap-4 flex-shrink-0">
        ...
        <div className="flex items-center gap-3 shrink-0 flex-wrap justify-end">
          {/* Calendar Controls — desktop only in header; mobile moves below */}
          {view !== 'items' && (
            <div className="hidden md:flex items-center gap-3 bg-neuro-element px-4 py-2 rounded-neuro-lg shadow-neuro-flat">
          ...
          {view !== 'items' && (
            <Button variant="primary" onClick={handleNewReservation} className="!py-2.5 text-sm !font-bold min-w-[11rem] text-center hidden sm:block">
              + New Reservation
            </Button>
          )}
        </div>
      ...
      {/* Mobile: full-width New Reservation button */}
      {view !== 'items' && (
        <Button variant="primary" onClick={handleNewReservation} className="!py-2.5 text-sm !font-bold w-full text-center sm:hidden flex-shrink-0">
          + New Reservation
        </Button>
      )}

      {/* Mobile: month navigator row — sits directly above the calendar */}
      {view !== 'items' && (
        <div className="md:hidden flex items-center gap-3 bg-neuro-element px-4 py-2 rounded-neuro-lg shadow-neuro-flat flex-shrink-0">
          <div className="flex items-center gap-1">
            <button
              onClick={handlePrev}
              className="p-1.5 rounded-neuro-sm text-neuro-secondary hover:text-neuro-primary hover:bg-white/50 transition-colors"
          ...
          <h2 className="text-sm font-semibold text-neuro-primary flex-1 text-center">
            {currentDateTitle}
          </h2>
